...
def parse_args():
    parser = argparse.ArgumentParser(description="Finetune a transformers model on an instruction-following task.")
    parser.add_argument(
        "--model_type",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--selection_metric", type=str, default="neg_perplexity", help="Metric·for·selection·criterion."
    )
    ...
    parser.add_argument("--trainable_modules", type=str, default="", help="Set trainable modules, others will be set as `requires_grad=False`.")
...
def main():
    ...
    # Initialize, then rewrite or add kwargs of original confi.
    ...
    if args.trainable_modules:
        ...
        # Set a subset of trainable parameters.
        for n, p in model.named_parameters():
            if not any(tm in n for tm in trainable_modules):
    ...
    num_saved_steps = 0
    if args.resume:
        num_saved_steps = int(os.path.basename(args.model_name_or_path).split("-")[1])
