...
def parse_args():
    parser = argparse.ArgumentParser(description="Pretraining a transformers model with LoRA.")
    parser.add_argument(
        "--model_type",
        type=str,
        required=True,
...
def main():
    ...
    ds_engine.save_checkpoint(last_dir)
    # Save. Weights might be a placeholder in zero3 and need a gather.
    if ds_engine.zero_optimization_partition_weights():
        state_dict = ds_engine._zero3_consolidated_16bit_state_dict()
    else:
    ...
    if is_main:
        ...
        config.save_pretrained(last_dir)
